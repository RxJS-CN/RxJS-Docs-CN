...
/**
 * 分支源 Observable 的值作为嵌套 Observable，使用工厂函数返回的关闭 Observables 来决定何时开启新的窗口。
 *
 ...
 *
 * 返回一个发出从源 Observable 收集到数据的窗口 Observable。 输出 Observable 发出连接的，非重叠的窗口。
 * 它释放当前窗口并且打开新的窗口当由指定 `closingSelector` 函数返回的 Observable
 * 发出数据时。当输出 Observable 被订阅的时候立马开启第一个窗口。
 *
 * @example <caption>仅仅发出前两个点击事件在每个窗口的[1-5]随机秒数</caption>
 ...
 * 该 Observable 发出信号(`next` 或者 `complete`)决定何时关闭前一个窗口，开启新一个窗口。
